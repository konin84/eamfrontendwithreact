...
export default function ClientsList() {
  ...
  const allClients = () => {
    userService.userList().then((res) => {
      ...
      setRecord(res.data.filter((x) => x.role === "CLIENT"));
    });
  };
  ...
  const addOrEdit = (realtor, resetForm) => {
    if (realtor.id) {
      userService.updateClient(realtor.id, realtor).then((res) => {
        console.log(res.data);
        setNotify({
          isOpen: true,
          message: "Client's Details Updated Successfully",
          type: "success",
        });
        resetForm();
  ...
  return (
    <div>
      ...
      >
        <ClientForm recordForEdit={recordForEdit} addOrEdit={addOrEdit} />
      </Popup>
